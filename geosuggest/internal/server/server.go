...
type Server struct {
	geosuggestpb.UnimplementedAddressSuggestServiceServer
	Api *suggest.Api
}

func (server *Server) Suggest(ctx context.Context, req *geosuggestpb.QueryRequest) (*geosuggestpb.SuggestResponse, error) {
	const retries = 3
	const delay = 1
	...
	suggestAddress := stability.Retry[suggest.RequestParams, []*suggest.AddressSuggestion](
		func(ctx context.Context, params suggest.RequestParams) ([]*suggest.AddressSuggestion, error) {
			return server.Api.Address(ctx, &params)
		}, retries, delay,
	)
